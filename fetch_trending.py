...
def upload_to_gitee(md_content: str):
    """通过 Gitee API 直接上传 Markdown 文件到 Obsidian 仓库"""
    token = os.environ.get('GITEE_TOKEN', '')
    repo = os.environ.get('GITEE_REPO', '')
    branch = os.environ.get('GITEE_BRANCH', 'master')

    if not token or not repo:
        print("⚠️ GITEE_TOKEN 或 GITEE_REPO 未配置，跳过 Gitee 同步")
        return False
    ...
    print(f"📂 上传到 Gitee: {repo}/{file_path}")

    try:
        # 先检查文件是否已存在
        check_resp = requests.get(api_url, params={
            'access_token': token,
            'ref': branch,
        }, timeout=15)
        ...
        if check_resp.status_code == 200:
            ...
            print(f"  📝 文件已存在，更新中 (sha: {existing_sha[:8]}...)")

        resp = requests.post(api_url, json=payload, timeout=30)

        if resp.status_code in (200, 201):
            result = resp.json()
            print(f"✅ 已推送到 Gitee: {result.get('content', {}).get('html_url', 'N/A')}")
            return True
        else:
            print(f"❌ Gitee API 错误 ({resp.status_code}): {resp.text[:300]}")
            return False
...
def main():
    ...
    # 4. 通过 Gitee API 上传到 Obsidian 仓库
    print("📂 同步到 Gitee Obsidian 仓库...")
    upload_to_gitee(md_content)
